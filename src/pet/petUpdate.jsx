  ...
  api
    ...
    .then(tasksList => {
      ...
      tasksList.forEach(task =>
        api.editTask(uid, { id: task.id, date: task.date, penalized: true })
      );
    })
    .catch(console.error);
}

export function checkAge({ birthday, health, stage }) {
  const petAge = Math.floor((new Date() - new Date(birthday)) / (1000 * 3600 * 24));

  if (stage === 'adult') {
    return { stage: stage, health: health };
  } else if (petAge >= 7) {
